...
//una función === una tarea (hacer que una fn haga una sola cosa)


/*EJ: Para buscar datos de una API creamos una función que hace un fetch
esto de orden superior no tiene nada.*/
const url = "https://jsonplaceholder.typicode.com/posts" //endpoint (de aca sacamos los datos)

const fetchData = async(url) => {                          //fetch en gral corre en el navegador (hay un paquete para correrlo en node, pero mhe..)
    const rawData = await fetch(url);
    const parsedData = await rawData.json();  //vienen los datos que hay q parseas a un objeto valido de js para poder iterar
    console.log(parsedData)
}
fetchData(url)

    /////////////
    /* En el paradigma funcional podemos pasarle una función a otra como parámetro. 
    Entonces haremos una función de orden superior (getData), que recibirá datos de una API con fetch. getData 
    recibirá como parámetros la url y una función de primer orden como callback. 
    Con esto logramos mayor abstracción, esto quedará claro un poco más adelante */
